...
备注：参数不能全为空,不会删除文件
*********************************************************************/
BOOL CXStorage_SQLite::XStorage_SQLite_FileDelete(LPCTSTR lpszBuckKey /* = NULL */, LPCTSTR lpszFile /* = NULL */, LPCTSTR lpszHash /* = NULL */)
{
    XStorage_IsErrorOccur = FALSE;
    ...
    int nListCount = 0;
    XSTORAGECORE_DBFILE** ppSt_ListFile;
    if (!XStorage_SQLite_FileQuery(&ppSt_ListFile, &nListCount, NULL, NULL, lpszBuckKey, lpszFile, lpszHash))
    {
        return FALSE;
    }
    //轮训查找删除
...
*********************************************************************/
BOOL CXStorage_SQLite::XStorage_SQLite_FileQuery(XSTORAGECORE_DBFILE*** pppSt_ListFile, int* pInt_ListCount, LPCTSTR lpszTimeStart /* = NULL */, LPCTSTR lpszTimeEnd /* = NULL */, LPCTSTR lpszBuckKey /* = NULL */, LPCTSTR lpszFile /* = NULL */, LPCTSTR lpszHash /* = NULL */, LPCTSTR lpszTableName /* = NULL */)
{
    XStorage_IsErrorOccur = FALSE;
    ...
    else
    {
		...
		TCHAR** pptszFileResult;
		if (DataBase_SQLite_GetTable(xhSQL, tszSQLStatement, &pptszFileResult, &nLine, &nRow))
		{
			int nFiled = nRow;
			//循环获取所有查找到的文件
			for (int i = 0; i < nLine; i++)
        ...
        for (int i = 0; i < nLine; i++)
        {
            ...
            if (XStorage_SQLite_TimeMonth(pptszResult[i], &nTimeMonth))
            {
                if (nTimeMonth > m_nTimeMonth)
                {
                    ...
                    int nListCount = 0;
                    XSTORAGECORE_DBFILE** ppSt_ListFile;
                    XStorage_SQLite_FileQuery(&ppSt_ListFile, &nListCount, NULL, NULL, NULL, NULL, NULL, pptszResult[i]);
                    for (int i = 0; i < nListCount; i++)
                    {
                        //删除文件
